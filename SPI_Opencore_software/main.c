...
int main() {
	...
	spi_opencore = virtual_base + ( ( unsigned long  )( ALT_LWFPGASLVS_OFST + SPI_CORES_0_BASE ) & ( unsigned long)( HW_REGS_MASK ) );
	
	*((uint32_t*)spi_opencore + 0x14) = 0xe;
	*((uint32_t*)spi_opencore + 0x0) = 0x95;
	*((uint32_t*)spi_opencore + 0x10) = 0x2208;
	*((uint32_t*)spi_opencore + 0x18) = 0x1;
	*((uint32_t*)spi_opencore + 0x10) = 0x2308;
	usleep(100000);
	printf("Rx: %x\n", *((uint32_t*)spi_opencore + 0x0));
	/*int i = 0;
	for(i = 0; i < 255; i++){
		*((uint32_t*)spi_opencore + 0x14) = 0xe;
		*((uint32_t*)spi_opencore + 0x0) = i;
		*((uint32_t*)spi_opencore + 0x10) = 0x2208;
		*((uint32_t*)spi_opencore + 0x18) = 0x1;
		*((uint32_t*)spi_opencore + 0x10) = 0x2308;
		usleep(100000);
		//printf("Divider: %x\n", *((uint32_t*)spi_opencore + 0x14));
		printf("Rx: %x\n", *((uint32_t*)spi_opencore + 0x0));
		//printf("CTRL: %x\n", *((uint32_t*)spi_opencore + 0x10));
		//printf("SS: %x\n", *((uint32_t*)spi_opencore + 0x18));
	}*/
